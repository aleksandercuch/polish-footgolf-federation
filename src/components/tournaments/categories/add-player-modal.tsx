...
export const AddPlayerModal: FC<componentProps> = ({
  tournament,
  tournamentId,
  category,
}) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      tournamentToUpdate.categories[index].players.push({
        id: createId(),
        categoryId: tournamentToUpdate.categories[index].id,
  ...
  return (
    <div className={cn(classes.modalWrapper, classes.addPlayerWrapper)}>
      <Image
        ...
        src={CrossIcon}
        alt="test"
        onClick={() => handleModal("Close Modal")}
      />
      <form className={classes.form} onSubmit={formik.handleSubmit}>
        <div>
          <h2 aria-hidden="true" className={classes.form__header}>
            {t("HEADERS.playerData")}
          </h2>
          <hr />
          <div className={classes.form__table}>
            <label>
              {formik.touched.firstName && formik.errors.firstName && (
                <div className={classes.form__errorSign}>*</div>
              )}
              {t("AUTH.firstName")}
            </label>
            <input
