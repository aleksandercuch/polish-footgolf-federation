...
import { Itournaments, Icategories, Iplayers } from "@/components/types/tournaments-types/tournament-types";
...
// TODO fix context errors
    ...
    const userSignedToCategory = (category: Icategories): boolean => {
      ...
      tournament && tournament.categories[index].players.forEach(player => {
                ...
                {Object.keys(user).length != 0 && (
                  <div className={classes.editButtonWrapper}>
                    ...
                    {userSignedToCategory(renderedCellValue.row.original) ? (
                      <Button 
                        content={t("BUTTONS.signedUp")}  
                        disabled
                      />
                    ) : (
                      <Button 
                        content={t("BUTTONS.signUp")} 
                        onClickFunc={() => assignLoggedUser(renderedCellValue.row.original)} 
                      />
                    )}
                    <Button content={t("BUTTONS.addPlayerLong")} 
                      onClickFunc={() => handleModal(
                      <AddPlayerModal
                        tournament={tournament} 
                        tournamentId={props.tournamentId}
